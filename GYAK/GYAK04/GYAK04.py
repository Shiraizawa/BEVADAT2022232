...
# %%
def get_column(test_df:pd.DataFrame, column_name):
    new_df=test_df.copy()
...
def get_top_two(test_df:pd.DataFrame):
    new_df=test_df.copy()
    new_df=new_df.sort_values('area', ascending=False)
    return new_df[:2]
...
def plot_area(test_df:pd.DataFrame):
    new_df=test_df.copy()
    fig, ax = plt.subplots()
    ax.pie(new_df['area'],labels=new_df['country'],labeldistance=0.8)
